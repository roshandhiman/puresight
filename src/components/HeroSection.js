...
import { ArrowRight, Shield, Sun, Activity, Waves } from 'lucide-react';
...
export default function HeroSection() {
  ...
  return (
    <section ref={heroRef} className="relative" style={{ zIndex: 2 }}>

      {/* ── ABOVE THE FOLD: Full viewport, centered content ── */}
      <div className="min-h-[100svh] flex flex-col items-center justify-center pt-24 pb-20 px-4 sm:px-6 relative overflow-hidden">
        ...
        {/* Main Headline — stagger per word */}
        ...
        <motion.p
          ...
          className="text-center text-xl md:text-2xl text-gray-400 max-w-2xl mb-14 leading-relaxed"
        ...
        >
          <MagneticButton href="/dashboard" className="btn-primary px-10 py-4 group text-base font-bold">
            View Live Dashboard
            <ArrowRight className="w-5 h-5 transition-transform group-hover:translate-x-1.5" />
          </MagneticButton>
          <MagneticButton href="/request-demo" className="btn-ghost px-10 py-4 text-base font-bold">
            Request Demo
          </MagneticButton>
        </motion.div>
      ...
      {/* ── BELOW THE FOLD: Dashboard Mockup — appears on scroll ── */}
      <div
        ref={mockupRef}
        onMouseMove={handleMockupMouseMove}
        onMouseLeave={handleMockupMouseLeave}
        className="px-4 sm:px-6 pb-32 max-w-5xl mx-auto w-full"
      >
        <Link href="/dashboard" className="block cursor-pointer group/mockup">
          <motion.div
          ...
          >
            ...
            >
              {/* Dashboard card */}
              <div
                className="bg-white rounded-3xl p-6 md:p-8 relative overflow-hidden border border-gray-100/80 group-hover/mockup:border-blue-200 transition-all duration-500"
              ...
              {/* Floating badge — left */}
              <motion.div
                animate={mockupInView ? { y: [0, -12, 0] } : {}}
                transition={{ duration: 4.5, repeat: Infinity, ease: 'easeInOut' }}
                className="absolute -left-16 top-1/3 bg-white rounded-2xl p-5 hidden md:block border border-gray-100 shadow-2xl shadow-black/8"
                style={{ minWidth: 185 }}
              >
                <div className="text-[10px] text-gray-400 mb-2 font-black uppercase tracking-[0.2em]">Next Service</div>
                <div className="flex items-center gap-2.5">
                  <div className="w-2.5 h-2.5 rounded-full bg-blue-500 animate-pulse flex-shrink-0" />
                  <span className="text-lg font-black text-gray-900">43 days away</span>
                </div>
              </motion.div>

              {/* Floating badge — right */}
              <motion.div
                animate={mockupInView ? { y: [0, 10, 0] } : {}}
              ...
              <motion.div
                animate={mockupInView ? { y: [0, -8, 0] } : {}}
                transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
                className="absolute -left-8 bottom-8 bg-blue-600 rounded-2xl p-3.5 hidden md:block shadow-xl shadow-blue-500/30"
                style={{ minWidth: 145 }}
              >
